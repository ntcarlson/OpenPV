...
int RunningAverageLayer::communicateInitInfo(std::shared_ptr<CommunicateInitInfoMessage const> message) {
...
void RunningAverageLayer::ioParam_numImagesToAverage(enum ParamsIOFlag ioFlag){
   ioParamValue(ioFlag, name, "numImagesToAverage", &numImagesToAverage, numImagesToAverage);
}
