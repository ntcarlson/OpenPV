...
void Retina::ioParam_spikingFlag(enum ParamsIOFlag ioFlag) {
   ioParamValue(ioFlag, name, "spikingFlag", &spikingFlag, true);
}
...
void Retina::ioParam_foregroundRate(enum ParamsIOFlag ioFlag) {
   ...
   ioParamValue(ioFlag, name, "foregroundRate", &probStimParam, 1.0f);
}
...
void Retina::ioParam_backgroundRate(enum ParamsIOFlag ioFlag) {
   ...
   ioParamValue(ioFlag, name, "backgroundRate", &probBaseParam, 0.0f);
...
void Retina::ioParam_beginStim(enum ParamsIOFlag ioFlag) {
   ioParamValue(ioFlag, name, "beginStim", &rParams.beginStim, 0.0);
}

void Retina::ioParam_endStim(enum ParamsIOFlag ioFlag) {
   ioParamValue(ioFlag, name, "endStim", &rParams.endStim, (double) FLT_MAX);
   if (ioFlag == PARAMS_IO_READ && rParams.endStim < 0) rParams.endStim = FLT_MAX;
}

void Retina::ioParam_burstFreq(enum ParamsIOFlag ioFlag) {
   ioParamValue(ioFlag, name, "burstFreq", &rParams.burstFreq, 1.0f);
}

void Retina::ioParam_burstDuration(enum ParamsIOFlag ioFlag) {
   ioParamValue(ioFlag, name, "burstDuration", &rParams.burstDuration, 1000.0f);
}

void Retina::ioParam_refractoryPeriod(enum ParamsIOFlag ioFlag) {
   assert(!getParams()->presentAndNotBeenRead(name, "spikingFlag"));
   if (spikingFlag){
      ioParamValue(ioFlag, name, "refractoryPeriod", &rParams.refractory_period, (float) REFRACTORY_PERIOD);
   }
}
...
void Retina::ioParam_absRefractoryPeriod(enum ParamsIOFlag ioFlag) {
   assert(!getParams()->presentAndNotBeenRead(name, "spikingFlag"));
   if (spikingFlag){
      ioParamValue(ioFlag, name, "absRefractoryPeriod", &rParams.abs_refractory_period, (float) ABS_REFRACTORY_PERIOD);
   }
}
