...
namespace PV {
   ...
   int InputLayer::scatterInput(int batchIndex) {
      ...
      const int rank = getCommunicator()->commRank();
   ...
   void InputLayer::fitBufferToLayer(Buffer &buffer) {
      pvAssert(getCommunicator()->commRank() == 0);
      const PVLayerLoc *loc = getLayerLoc();
      const PVHalo *halo = &loc->halo;
   ...
   void InputLayer::populateFileList() {
      if (mUsingFileList && getCommunicator()->commRank() == 0) {
         std::string line;
         mFileList.clear();
   ...
   int InputLayer::checkpointRead(const char * cpDir, double const * timef) {
   ...
   int InputLayer::checkpointWrite(bool suppressCheckpointIfConstant, const char * cpDir, double timestamp) {
      int status = HyPerLayer::checkpointWrite(suppressCheckpointIfConstant, cpDir, timestamp);
      if (getCommunicator()->commRank() == 0) {
         writeArrayToFile(cpDir, getName(), "FrameNumbers", getCommunicator(), static_cast<int*>(mBatchIndexer->getIndices().data()), parent->getNBatch(), parent->getVerifyWrites());
      }
      else {
         // This is just to line up MPI calls
         int *garbage = static_cast<int*>(calloc(parent->getNBatch(), sizeof(int)));
         writeArrayToFile(cpDir, getName(), "FrameNumbers", getCommunicator(), garbage, parent->getNBatch(), parent->getVerifyWrites());
         free(garbage);
      }
   ...
   void InputLayer::ioParam_start_frame_index(enum ParamsIOFlag ioFlag) {
      ...
      this->ioParamArray(ioFlag, this->getName(), "start_frame_index", &paramsStartFrameIndex, &length);
   ...
   void InputLayer::ioParam_skip_frame_index(enum ParamsIOFlag ioFlag) {
      ...
      this->ioParamArray(ioFlag, this->getName(), "skip_frame_index", &paramsSkipFrameIndex, &length);
