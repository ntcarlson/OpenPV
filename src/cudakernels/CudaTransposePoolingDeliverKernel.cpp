...
void CudaTransposePoolingDeliverKernel::setArgs(
   ...
   mDataStore = (float*) dataStoreBuffer->getDevicePointer();
   mCudnnDataStore = new CudaBuffer(dataStoreBuffer->getSize(), device);
   ...
   float * gSynHead = (float*) gSynBuffer->getDevicePointer();
   mGSyn = &gSynHead[channel*numGSynNeuronsAcrossBatch];
   mCudnnGSyn = new CudaBuffer(numGSynNeuronsAcrossBatch*sizeof(float), device);
   ...
   mOrigConnDataStore = (float*) origConnDataStoreBuffer->getDevicePointer();
   mCudnnOrigConnDataStore = new CudaBuffer(origConnDataStoreBuffer->getSize(), device);
   ...
   float * origConnGSynHead = (float*) origConnGSynBuffer->getDevicePointer();
   mOrigConnGSyn = &origConnGSynHead[channel*numOrigConnGSynNeuronsAcrossBatch];
   mCudnnOrigConnGSyn = new CudaBuffer(numOrigConnGSynNeuronsAcrossBatch*sizeof(float), device);
}
...
int CudaTransposePoolingDeliverKernel::do_run() {
   ...
   float * cudnnDataStorePointer = (float*) mCudnnDataStore->getDevicePointer();
   ...
   float * cudnnGSynPointer = (float*) mCudnnGSyn->getDevicePointer();
   //Ceil to get all neurons
   ...
   float * cudnnOrigConnDataStorePointer = (float*) mCudnnOrigConnDataStore->getDevicePointer();
   ...
   float * cudnnOrigConnGSynPointer = (float*) mCudnnOrigConnGSyn->getDevicePointer();
   //Ceil to get all neurons
