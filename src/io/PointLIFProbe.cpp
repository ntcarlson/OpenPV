...
void PointLIFProbe::ioParam_writeStep(enum ParamsIOFlag ioFlag) {
   ...
   ioParamValue(ioFlag, getName(), "writeStep", &writeStep, writeStep, true/*warnIfAbsent*/);
}
...
int PointLIFProbe::calcValues(double timevalue) {
   ...
   if( xLocLocal >= 0 && xLocLocal < nx &&
      ...
      if(getCommunicator()->commRank()!=0){
         MPI_Send(valuesBuffer, NUMBER_OF_VALUES, MPI_DOUBLE, 0, 0, getCommunicator()->communicator());
      }
   }
   ...
   if(getCommunicator()->commRank()==0){
      ...
      if(srcRank != 0){
         MPI_Recv(valuesBuffer, NUMBER_OF_VALUES, MPI_DOUBLE, srcRank, 0, getCommunicator()->communicator(), MPI_STATUS_IGNORE);
      }
   }
   return PV_SUCCESS;
}
...
{
   if (getCommunicator()->commRank()==0 && timed >= writeTime) {
