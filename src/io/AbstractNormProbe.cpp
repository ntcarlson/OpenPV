...
void AbstractNormProbe::ioParam_maskLayerName(enum ParamsIOFlag ioFlag) {
   ioParamString(ioFlag, name, "maskLayerName", &maskLayerName, NULL, false/*warnIfAbsent*/);
}
...
int AbstractNormProbe::calcValues(double timeValue) {
   ...
   MPI_Allreduce(MPI_IN_PLACE, valuesBuffer, getNumValues(), MPI_DOUBLE, MPI_SUM, getCommunicator()->communicator());
   return PV_SUCCESS;
}
